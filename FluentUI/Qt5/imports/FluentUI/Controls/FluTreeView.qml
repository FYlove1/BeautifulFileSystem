...
Rectangle {
    ...
    Component{
        ...
        Item{
            ...
            function toggle(){
                // 步骤1：确认未保存编辑
                if (d.editPosition) {
                    if (!FluMessageBox.confirm("当前有未保存的编辑内容，是否放弃？")) {
                        return; // 取消操作
                    }
                }
                ...
                console.log("control exists:", typeof control === "object")
                // 步骤3：关闭编辑器并刷新视图
                control.closeEditor();
                delay_force_layout.restart(); // 触发布局更新
            }
            MouseArea{
                id:item_mouse
    ...
    function collapse(rowIndex){
        ...
        control.closeEditor(); // 新增：折叠时关闭编辑
    }
    function expand(rowIndex){
        tree_model.expand(rowIndex)
        control.closeEditor(); // 新增：展开时关闭编辑
    }
    function allExpand(){
    ...
    function closeEditor(){
        ...
        // 销毁编辑器组件
        loader_edit.sourceComponent = undefined
        loader_edit.display = undefined
        loader_edit.width = 0
        loader_edit.height = 0
